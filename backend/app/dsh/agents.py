...
AGENTS: list[AgentDefinition] = [
    ...
    AgentDefinition(
        ...
        system_prompt=(
            ...
            "但「问结论」是 qa 不是 analyze：询问已完成任务的结论/结果/解释"
            "（『刚才结论如何』『那个任务的结果讲讲』）由问答助手引用历史作答；"
            "只有要求继续/重新分析或生成报告才是 analyze/full。\n"
    ...
    AgentDefinition(
        ...
        system_prompt=(
            ...
            "当提问涉及具体代码/接口/实现（如「某某服务里 X 的逻辑」）：禁止凭记忆作答，"
            "必须用 glob/grep/read 工具在「源码工作区」里实际检索（path 用绝对路径、"
            "以工作区开头），再基于检索到的内容回答，并标注出处（文件路径:行号）。\n"
            "工作区内检索不到就明确说未找到，绝不编造；只有确实需要完整证据链与报告时，"
            "才建议用户切换到「需求分析/全流程」模式。\n"
